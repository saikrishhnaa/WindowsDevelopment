...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMSG, WPARAM wParam, LPARAM lParam)
{
	...
	{
		case WM_CREATE:
			hr = CoCreateInstance(&CLSID_SumSubstract, NULL, CLSCTX_INPROC_SERVER, &IID_ISum, (void**)&pISum); //C don't have reference but have pointer so passing address instead of reference
			if (FAILED(hr))
			{
			...
			pISum->lpVtbl->QueryInterface(pISum , &IID_ISubstract, (void**)&pISubstract);
			pISubstract->lpVtbl->SubstractionOfTwoIntegers(pISubstract, n1, n2, &sub);
